...
class ScamShieldAgent:
    ...
    def _load_core_prompt(self):
        if self._core_prompt is None:
            full = self._load_full_prompt()
            # Use first 30000 chars (~7500 tokens) as core
            # This covers: scam types, risk scoring, response format, UK data
            self._core_prompt = full[:30000]
