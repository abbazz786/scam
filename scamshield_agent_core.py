...
{{"verdict":"SCAM or SUSPICIOUS or UNVERIFIED or LEGITIMATE","riskScore":50,"summary":"What this target is in 2 sentences","redFlags":["flag1"],"greenFlags":["signal1"],"evidenceFound":["point1"],"ukReports":["pattern1"],"recommendation":"What to do","reportTo":"Relevant UK authority and URL"}}"""
    ...
    def ask_question(self, question, user_name="there"):
        response = self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": self._load_core_prompt()},
                {"role": "user", "content": "Answer in plain English only — no JSON, no bullet points. Question: " + question[:1500]}
            ]
        )
        raw = response.choices[0].message.content.strip()
        # Strip any accidental JSON
        if raw.startswith('{') or raw.startswith('```'):
            return "Yes, this is a scam. " + question[:80] + " — do not forward any codes or send money. Report to reporting.reportfraud.police.uk"
        return raw
